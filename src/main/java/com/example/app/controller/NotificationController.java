...
public class NotificationController {
  ...
  public ResponseEntity<Notification> create(@Valid @RequestBody Notification value) {
    return ResponseEntity.ok(repository.save(value));
  }

  @GetMapping
  @Operation(summary = "List notification records")
  public ResponseEntity<Page<Notification>> list(@PageableDefault(size = 20) Pageable pageable) {
  ...
  public ResponseEntity<Notification> update(
      @PathVariable Long id, @Valid @RequestBody Notification value) {
    return repository
        .findById(id)
        .map(
            old -> {
              value.setMessage(value.getMessage());
              return ResponseEntity.ok(repository.save(value));
            })
        .orElse(ResponseEntity.notFound().build());
  }
